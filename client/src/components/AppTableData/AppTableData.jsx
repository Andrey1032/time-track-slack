...
import { currentCalendarData } from "../../assets/store/slices/employeesSlice";
import {
...
export default function AppTableData({
    ...
    const converTimeInHoursMinutes = (time) => {
    ...
    const exportFile = () => {
        ...
        if (dataOtchet) {
            ...
            dataOtchet
                .map((otc) => XLSX.utils.aoa_to_sheet(otc))
                .map((w, w_i) =>
                    XLSX.utils.book_append_sheet(wb, w, `${data[w_i].title}`)
                ); //собираем все страницу в одну книгу

            XLSX.writeFile(
                wb,
                "Табель рабочего времени Май 2024 г. Отдел разработки.xlsx"
            ); // выгрузка файла
        }
    };
        ...
        <div className={role ? "table__data manager" : "table__data "}>
            ...
            <div className="table__container">
                ...
                <div className="table">
                    <div
                        className={
                            role ? "table__titles manager" : "table__titles "
                        }
                    >
                        ...
                        {data[0] &&
                            data[0]?.map((data, data_id) => (
                                <>
                                    ...
                                    <ItemWorkTime
                                        data={data.reworked_time?.map((rt) => ({
                                            time: converTimeInHoursMinutes(
                                                rt.time
                                            ),
                                            comment: rt.comment,
