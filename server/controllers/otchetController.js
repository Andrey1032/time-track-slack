...
class OtchetController {
    async getOtchet(req, res) {
        ...
        const id_department = Number(req.query.id_department);
        const month = Number(req.query.month);
        const year = Number(req.query.year);

        console.log("!!!!!!!!!!!!!!QUERY!!!!!!!!!!!!!!!!!!", req.query);
        let department, departmentF;
        ...
        try {
            if (id_department) {
                ...
                department = await Department.findOne({
                    ...
                    attributes: ["id_department", ["name_department", "title"]],
                });
            } else {
                let dates = JSON.parse(
                    JSON.stringify(
            ...
            if (Array.isArray(departmentF)) {
                ...
                departmentF.forEach(async (department) => {
                    departmentForOtchet.push({
                        ...
                        subtitle: `Табель рабочего времени ${
                            months[month - 1]
                        } ${year} г. ${department.title}`,
                        underworking: [],
                        reworking: [],
                    });
                });
                ...
                departmentF.forEach((department, index) => {
                    department.users.forEach(async (user) => {
                        let massivDatesRework = new Array(
                            lastDayCurrentMonth.getDate()
                        ).fill(null);
                        let massivDatesUnderwork = new Array(
                            lastDayCurrentMonth.getDate()
                        ).fill(null);
                        ...
                        user.reworkings.forEach((rework) => {
                            ...
                            let time = sumTime(
                                convertTime(rework.time_reworking),
                                convertTime(massivDatesRework[day - 1])
                            );
                            massivDatesRework[day - 1] = `${Math.floor(
                                time / 60
                            )}:${
                                time % 60 < 10
                                    ? `0${time % 60}`
                        ...
                        user.underworkings.forEach((rework) => {
                            let day = new Date(rework.calendar.date).getDate();
                            let time = sumTime(
                                convertTime(rework.time_underworking),
                                convertTime(massivDatesUnderwork[day - 1])
                            );
                            massivDatesUnderwork[day - 1] = `${Math.floor(
                                time / 60
                            )}:${
                                time % 60 < 10
                                    ? `0${time % 60}`
                        ...
                        let writtenRe = user.writing_off_times
                            .toReversed()
                            .find((type) => type.typeOfWorkIdTypeOfWork === 1);
                        let writtenUnder = user.writing_off_times
                            .toReversed()
                            .find((type) => type.typeOfWorkIdTypeOfWork === 2);

                        departmentForOtchet[index].underworking.push({
                            name: `${user.surname} ${user.name[0]}.`,
                            missed:
                                writtenUnder?.new_value ||
                                user.underworking_months[0]
                ...
                departmentF.users.forEach(async (user) => {
                    ...
                    let writtenRe =
                        user.writing_off_times.length > 0
                            ? user.writing_off_times?.toReversed()
                            : user.writing_off_times.find(
                                  (type) => type.typeOfWorkIdTypeOfWork === 1
                              );
                    let writtenUnder =
                        user.writing_off_times.length > 0
                            ? user.writing_off_times?.toReversed()
                            : user.writing_off_times.find(
                                  (type) => type.typeOfWorkIdTypeOfWork === 2
                              );
