...
class ReworkingMonthController {
    async createReworkingMonth(req, res){
        ...
        try {
            const reworkingMonth = await Reworking_month.create({month, year, time_reworking_month, userIdUser})
    ...
    async updateReworkingMonth(req, res){
        ...
        const {month, year, time_reworking_month, userIdUser} = req.body
        try {
            const reworkingMonth = await Reworking_month.update({month, year, time_reworking_month, userIdUser}, {where: {id_reworking_month}})
            return res.json(reworkingMonth)
        } catch (error) {
            console.log(error)
