...
class UnderworkingMonthController {
    async createReworkingMonth(req, res){
        ...
        try {
            const underworkingMonth = await Underworking_month.create({month, year, time_underworking_month, userIdUser})
    ...
    async updateReworkingMonth(req, res){
        ...
        const {month, year, time_underworking_month, userIdUser} = req.body
        try {
            const underworkingMonth = await Underworking_month.update({month, year, time_underworking_month, userIdUser}, {where: {id_underworking_month}})
            return res.json(underworkingMonth)
        } catch (error) {
            console.log(error)
