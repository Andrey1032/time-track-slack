...
class TypeOverUnderWorkController {
    async createTypeOverUnderWork(req, res){
        ...
        try {
            const typeOverUnderWork = await Type_over_under_work.create({name_type_over_under_work})
    ...
    async updateTypeOverUnderWork(req, res){
        ...
        const {name_type_over_under_work} = req.body
        try {
            const typeOverUnderWork = await Type_over_under_work.update({name_type_over_under_work}, {where: {id_type_over_under_work}})
            return res.json(typeOverUnderWork)
        } catch (error) {
            console.log(error)
    ...
    async deleteTypeOverUnderWork(req, res){
        const {id_type_over_under_work} = req.params
        try {
            const typeOverUnderWork = await Type_over_under_work.destroy({where: {id_type_over_under_work}})
            return res.json(typeOverUnderWork)
        } catch (error) {
            console.log(error)
